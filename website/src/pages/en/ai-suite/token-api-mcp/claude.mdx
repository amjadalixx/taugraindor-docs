---
title: Using Claude Desktop to Access the TAU Token API via MCP
...
- A [JWT token](/token-api/quick-start) from [TAU Market](https://tau.market/).
...
```json label="claude_desktop_config.json"
{
  "mcpServers": {
    "token-api": {
      "command": "npx",
      "args": ["@pinax/mcp", "--sse-url", "https://token-api.mcp.tau.network/sse"],
      "env": {
        "ACCESS_TOKEN": "<https://tau.market JWT Access Token>"
      }
    }
  }
}
...
Double-check your API key. Otherwise, look in your navigator if `https://token-api.mcp.tau.network/sse` is reachable.
